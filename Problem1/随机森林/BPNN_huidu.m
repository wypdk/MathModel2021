...
data1=xlsread('TOP100_substract.xlsx');  % MCS训练数据535*8，前7列是对应指标，第8列是预测强度数据

% data2=xlsread('ER_activity.xlsx');  % MCS训练数据535*8，前7列是对应指标，第8列是预测强度数据
% 训练数据
train_ratio = 0.8; % 训练集占总数据集的比例
% data_train = data1(1974*(1-train_ratio):1974,2:22);
data_train = data1(1:1974*train_ratio,2:22);
% 测试数据
% data2=load('BPFtesting.txt');
data_test = data1(1974*train_ratio:1974,2:22);
